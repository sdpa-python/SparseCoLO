%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [QMat0,cVect0,lbd,ubd,K0,L0] = triDiagQSDP2(nDim,randSeed,fileSaveSW); 

%
% Nonconvex tridiagonal QSDP with equality and inequality constraints
%
...
oneCoefMat(1,1) = 1; 
oneCoefMat(2:K0.s,2:K0.s) = -speye(K0.s-1,K0.s-1); 
QMat0 = [QMat0; reshape(oneCoefMat,1,K0.s*K0.s)];

% Inequality constraint 
L0.l = 1; 
oneCoefMat = sparse(K0.s,K0.s); 
oneCoefMat(1,2:K0.s) = -rand(1,K0.s-1); % -0.5*ones(1,K0.s-1); 
oneCoefMat(2:K0.s,1) = oneCoefMat(1,2:K0.s)';
QMat0 = [QMat0; reshape(oneCoefMat,1,K0.s*K0.s)];

% Convex Quadratic matrix inequality
L0.s = nDim; 
for i=1:nDim
    for j= 1:nDim
        if i==j 
